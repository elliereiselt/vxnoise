...
static __m128 VX_VECTORCALL hashCellX(__m128i seed, __m128 cellX, __m128 cellY) {
    ...
    __m128i hashX = vx::simd::sse42::partialJenkinsHash(seed, _mm_cvttps_epi32(rx));
    __m128 pointX = _mm_div_ps(_mm_cvtepi32_ps(hashX), _mm_set1_ps(1000000.0f));
    __m128 fractX = _mm_sub_ps(pointX, _mm_floor_ps(pointX));

    return _mm_add_ps(cellX, fractX);
}

static __m128 VX_VECTORCALL hashCellY(__m128i seed, __m128 cellX, __m128 cellY) {
    __m128 ry = vx::simd::sse42::dot_ps(cellX, cellY, _mm_set1_ps(269.5f), _mm_set1_ps(183.3f));
    __m128i hashY = vx::simd::sse42::partialJenkinsHash(seed, _mm_cvttps_epi32(ry));
    __m128 pointY = _mm_div_ps(_mm_cvtepi32_ps(hashY), _mm_set1_ps(1000000.0f));
    __m128 fractY = _mm_sub_ps(pointY, _mm_floor_ps(pointY));
...
static __m128 hashCellX(__m128i seed, __m128 cellX, __m128 cellY, __m128 cellZ) {
    ...
    __m128 rx = vx::simd::sse42::dot_ps(cellX, cellY, cellZ,
...
static __m128 hashCellY(__m128i seed, __m128 cellX, __m128 cellY, __m128 cellZ) {
    // Gonna be honest with ya here. Don't remember where the original x and y constants came from, the z constants were made up from keyboard smashing. Probably not good.
    __m128 ry = vx::simd::sse42::dot_ps(cellX, cellY, cellZ,
...
static __m128 hashCellZ(__m128i seed, __m128 cellX, __m128 cellY, __m128 cellZ) {
    // Gonna be honest with ya here. Don't remember where the original x and y constants came from, the z constants were made up from keyboard smashing. Probably not good.
    __m128 rz = vx::simd::sse42::dot_ps(cellX, cellY, cellZ,
                                       _mm_set1_ps(419.2f), _mm_set1_ps(371.9f), _mm_set1_ps(523.7f));
    __m128i hashZ = vx::simd::sse42::partialJenkinsHash(seed, _mm_cvttps_epi32(rz));
    __m128 pointZ = _mm_div_ps(_mm_cvtepi32_ps(hashZ), _mm_set1_ps(1000000.0f));
    __m128 fractZ = _mm_sub_ps(pointZ, _mm_floor_ps(pointZ));

    return _mm_add_ps(cellZ, fractZ);
}

__m128 vx::noise::CellularSSE42::sse42Get2d(__m128 x, __m128 y, __m128 scale) {
    x = _mm_div_ps(x, scale);
    y = _mm_div_ps(y, scale);

    __m128 fx = _mm_div_ps(x, _mm_set1_ps(16));
    __m128 fy = _mm_div_ps(y, _mm_set1_ps(16));
    __m128 ix = _mm_floor_ps(fx);
    __m128 iy = _mm_floor_ps(fy);
    ...
    for (int ignoreI = -1; ignoreI < 2; ++ignoreI) {
        ...
        for (int ignoreJ = -1; ignoreJ < 2; ++ignoreJ) {
            __m128 cellX = hashCellX(_sse42Seed, _mm_add_ps(ix, i), _mm_add_ps(iy, j));
            __m128 cellY = hashCellY(_sse42Seed, _mm_add_ps(ix, i), _mm_add_ps(iy, j));
            ...
            __m128 manhattanDistance = _mm_add_ps(vx::simd::sse42::abs_ps(_mm_sub_ps(cellY, fy)), vx::simd::sse42::abs_ps(_mm_sub_ps(cellX, fx)));
            ...
            lowestDistance = _mm_blendv_ps(lowestDistance, naturalDistance, distLTlowestDist);
            resultX = _mm_blendv_ps(resultX, cellX, distLTlowestDist);
            resultY = _mm_blendv_ps(resultY, cellY, distLTlowestDist);
    ...
    if (_noiseLookupSSE42 != nullptr) {
        return _noiseLookupSSE42->sse42Get2d(resultX, resultY);
    } else {
...
float vx::noise::CellularSSE42::get2d(float x, float y, float scale) {
...
__m128 vx::noise::CellularSSE42::sse42Get3d(__m128 x, __m128 y, __m128 z, __m128 scale) {
    x = _mm_div_ps(x, scale);
    y = _mm_div_ps(y, scale);
    ...
    for (int ignoreI = -1; ignoreI < 2; ++ignoreI) {
        ...
        for (int ignoreJ = -1; ignoreJ < 2; ++ignoreJ) {
            ...
            for (int ignoreK = -1; ignoreK < 2; ++ignoreK) {
                __m128 cellX = hashCellX(_sse42Seed, _mm_add_ps(ix, i), _mm_add_ps(iy, j), _mm_add_ps(iz, k));
                __m128 cellY = hashCellY(_sse42Seed, _mm_add_ps(ix, i), _mm_add_ps(iy, j), _mm_add_ps(iz, k));
                __m128 cellZ = hashCellZ(_sse42Seed, _mm_add_ps(ix, i), _mm_add_ps(iy, j), _mm_add_ps(iz, k));
                ...
                __m128 manhattanDistance =
                        _mm_add_ps(
                                _mm_add_ps(
                                        vx::simd::sse42::abs_ps(_mm_sub_ps(cellZ, fz)),
                                        vx::simd::sse42::abs_ps(_mm_sub_ps(cellY, fy))),
                                vx::simd::sse42::abs_ps(_mm_sub_ps(cellX, fx)));
                __m128 naturalDistance = _mm_add_ps(euclideanDistance, manhattanDistance);
                ...
                resultX = _mm_blendv_ps(resultX, cellX, distLTlowestDist);
                resultY = _mm_blendv_ps(resultY, cellY, distLTlowestDist);
                resultZ = _mm_blendv_ps(resultZ, cellZ, distLTlowestDist);
    ...
    if (_noiseLookupSSE42 != nullptr) {
        return _noiseLookupSSE42->sse42Get3d(resultX, resultY, resultZ);
    } else {
        alignas(16) float pResultX[4];
        alignas(16) float pResultY[4];
...
float vx::noise::CellularSSE42::get3d(float x, float y, float z, float scale) {
...
vx::aligned_array_2d<float> vx::noise::CellularSSE42::noise(float offsetX, float offsetY,
...
vx::aligned_array_3d<float> vx::noise::CellularSSE42::noise(float offsetX, float offsetY, float offsetZ,
