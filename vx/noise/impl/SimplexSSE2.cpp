...
#include "SimplexSSE2.hpp"

static __m128 VX_VECTORCALL sse2_grad(__m128i hash, __m128 x, __m128 y) {
    __m128i h = _mm_and_si128(hash, _mm_set1_epi32(7));
    __m128 hlt4 = _mm_cmplt_ps(_mm_cvtepi32_ps(h), _mm_set1_ps(4));
    __m128 u = vx::simd::sse2::blendv_ps(y, x, hlt4);
    __m128 v = vx::simd::sse2::blendv_ps(x, y, hlt4);

    __m128 h1 = _mm_cvtepi32_ps(_mm_cmpeq_epi32(_mm_and_si128(h, _mm_set_epi32(1, 1, 1, 1)), _mm_set1_epi32(1)));
    __m128 h2 = _mm_cvtepi32_ps(_mm_cmpeq_epi32(_mm_and_si128(h, _mm_set_epi32(2, 2, 2, 2)), _mm_set1_epi32(2)));
...
static __m128 VX_VECTORCALL sse2_grad(__m128i hash, __m128 x, __m128 y, __m128 z) {
    ...
    __m128 u = vx::simd::sse2::blendv_ps(y, x, hlt8);
    __m128 v = vx::simd::sse2::blendv_ps(vx::simd::sse2::blendv_ps(z, x, heq12or14), y, hlt4);
...
__m128 VX_VECTORCALL vx::noise::sse2SimplexGet2d(__m128i seed, __m128 x, __m128 y) {
    const __m128 sse2_F2 = _mm_set1_ps(F2);
    const __m128 sse2_G2 = _mm_set1_ps(G2);

    __m128 n0, n1, n2; // Noise contributions from the three corners

// Skew the input space to determine which simplex cell we're in
    __m128 s = _mm_mul_ps(_mm_add_ps(x, y), sse2_F2); // Hairy factor for 2D
    ...
    __m128 i = vx::simd::sse2::floor_ps(xs);
    __m128 j = vx::simd::sse2::floor_ps(ys);

    __m128 t = _mm_mul_ps(_mm_add_ps(i, j), sse2_G2);
    ...
    __m128 x1 = _mm_add_ps(_mm_sub_ps(x0, i1), sse2_G2); // Offsets for middle corner in (x,y) unskewed coords
    __m128 y1 = _mm_add_ps(_mm_sub_ps(y0, j1), sse2_G2);
    __m128 x2 = _mm_add_ps(_mm_sub_ps(x0, _mm_set1_ps(1.0f)), _mm_mul_ps(_mm_set1_ps(2.0f), sse2_G2)); // Offsets for last corner in (x,y) unskewed coords
    __m128 y2 = _mm_add_ps(_mm_sub_ps(y0, _mm_set1_ps(1.0f)), _mm_mul_ps(_mm_set1_ps(2.0f), sse2_G2));
    ...
    __m128i jj0h = vx::simd::sse2::partialJenkinsHash(seed, jj);
    __m128i jj1h = vx::simd::sse2::partialJenkinsHash(seed, jj1);
    __m128i jj2h = vx::simd::sse2::partialJenkinsHash(seed, jj2);

    __m128i ii0h = vx::simd::sse2::partialJenkinsHash(seed, _mm_add_epi32(ii, jj0h));
    __m128i ii1h = vx::simd::sse2::partialJenkinsHash(seed, _mm_add_epi32(ii1, jj1h));
    __m128i ii2h = vx::simd::sse2::partialJenkinsHash(seed, _mm_add_epi32(ii2, jj2h));

    __m128 g0 = sse2_grad(ii0h, x0, y0);
    __m128 g1 = sse2_grad(ii1h, x1, y1);
    __m128 g2 = sse2_grad(ii2h, x2, y2);
...
__m128 VX_VECTORCALL vx::noise::sse2SimplexGet3d(__m128i seed, __m128 x, __m128 y, __m128 z) {
    ...
    __m128 i = vx::simd::sse2::floor_ps(xs);
    __m128 j = vx::simd::sse2::floor_ps(ys);
    __m128 k = vx::simd::sse2::floor_ps(zs);
...
// TODO: Is this right?
    ...
    __m128i kk0h = vx::simd::sse2::partialJenkinsHash(seed, kk);
    __m128i kk1h = vx::simd::sse2::partialJenkinsHash(seed, kk1);
    __m128i kk2h = vx::simd::sse2::partialJenkinsHash(seed, kk2);
    __m128i kk3h = vx::simd::sse2::partialJenkinsHash(seed, kk3);

    __m128i jj0h = vx::simd::sse2::partialJenkinsHash(seed, _mm_add_epi32(jj, kk0h));
    __m128i jj1h = vx::simd::sse2::partialJenkinsHash(seed, _mm_add_epi32(jj1, kk1h));
    __m128i jj2h = vx::simd::sse2::partialJenkinsHash(seed, _mm_add_epi32(jj2, kk2h));
    __m128i jj3h = vx::simd::sse2::partialJenkinsHash(seed, _mm_add_epi32(jj3, kk3h));

    __m128i ii0h = vx::simd::sse2::partialJenkinsHash(seed, _mm_add_epi32(ii, jj0h));
    __m128i ii1h = vx::simd::sse2::partialJenkinsHash(seed, _mm_add_epi32(ii1, jj1h));
    __m128i ii2h = vx::simd::sse2::partialJenkinsHash(seed, _mm_add_epi32(ii2, jj2h));
    __m128i ii3h = vx::simd::sse2::partialJenkinsHash(seed, _mm_add_epi32(ii3, jj3h));

    __m128 g0 = sse2_grad(ii0h, x0, y0, z0);
    __m128 g1 = sse2_grad(ii1h, x1, y1, z1);
    __m128 g2 = sse2_grad(ii2h, x2, y2, z2);
    __m128 g3 = sse2_grad(ii3h, x3, y3, z3);
...
__m128 vx::noise::SimplexSSE2::sse2Get2d(__m128 x, __m128 y, __m128 scale) {
    return sse2SimplexGet2d(_sse2Seed, _mm_div_ps(x, scale), _mm_div_ps(y, scale));
}

float vx::noise::SimplexSSE2::get2d(float x, float y, float scale) {
    __m128 sse2Result = sse2SimplexGet2d(_sse2Seed,
...
__m128 vx::noise::SimplexSSE2::sse2Get3d(__m128 x, __m128 y, __m128 z, __m128 scale) {
    return sse2SimplexGet3d(_sse2Seed, _mm_div_ps(x, scale), _mm_div_ps(y, scale), _mm_div_ps(z, scale));
}

float vx::noise::SimplexSSE2::get3d(float x, float y, float z, float scale) {
    __m128 sse2Result = sse2SimplexGet3d(_sse2Seed,
                                         _mm_div_ps(_mm_set1_ps(x), _mm_set1_ps(scale)),
                                         _mm_div_ps(_mm_set1_ps(y), _mm_set1_ps(scale)),
...
vx::aligned_array_2d<float> vx::noise::SimplexSSE2::noise(float offsetX, float offsetY,
...
vx::aligned_array_3d<float> vx::noise::SimplexSSE2::noise(float offsetX, float offsetY, float offsetZ,
