#include "SimplexFractalSSE2.hpp"
#include "SimplexSSE2.hpp"

__m128 VX_VECTORCALL vx::noise::SimplexFractalSSE2::sse2Get2d(__m128 x, __m128 y, __m128 scale) {
    ...
    for (std::uint32_t octave = 0; octave < _octaves; ++octave) {
        __m128 noise = vx::noise::sse2SimplexGet2d(_sse2Seed,
        ...
        amplitude = _mm_mul_ps(amplitude, _sse2Persistence);
        frequency = _mm_mul_ps(frequency, _sse2Lacunarity);
    }

    return resultNoise;
}

__m128 VX_VECTORCALL vx::noise::SimplexFractalSSE2::sse2Get3d(__m128 x, __m128 y, __m128 z, __m128 scale) {
    __m128 frequency = _mm_set1_ps(1);
    __m128 amplitude = _mm_set1_ps(1);
    __m128 resultNoise = _mm_setzero_ps();

    for (std::uint32_t octave = 0; octave < _octaves; ++octave) {
        __m128 noise = vx::noise::sse2SimplexGet3d(_sse2Seed,
                                                   _mm_mul_ps(_mm_div_ps(x, scale), frequency),
                                                   _mm_mul_ps(_mm_div_ps(y, scale), frequency),
...
float vx::noise::SimplexFractalSSE2::get2d(float x, float y, float scale) {
...
float vx::noise::SimplexFractalSSE2::get3d(float x, float y, float z, float scale) {
...
vx::aligned_array_2d<float> vx::noise::SimplexFractalSSE2::noise(float offsetX, float offsetY,
...
vx::aligned_array_3d<float> vx::noise::SimplexFractalSSE2::noise(float offsetX, float offsetY, float offsetZ,
